...
def generate_pseudo_labels_zs(cas, label):
    ...
    # pseduo_labels = (outputs > threshold).float()
    ...
    for i in range(cas.shape[0]):
        ...
        # 形态学操作
        # 开门
        erode_1 = cv2.erode(kmeans_labels[i, :], np.ones((10,), np.uint8))
        dilated_1 = cv2.dilate(erode_1, np.ones((10,), np.uint8))

        # 关门
        # 先进行膨胀操作,连接临近区域
        dilated_2 = cv2.dilate(dilated_1, np.ones((50,), np.uint8))
        # 再进行腐蚀操作,去除噪声
        erode_2 = cv2.erode(dilated_2, np.ones((50,), np.uint8))
        kmeans_labels[i] = erode_2.reshape(cas.shape[2])
        # plt.plot(kmeans_labels[i])
        # plt.show()
        # print('')
