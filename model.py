...
import math
import time
from utils.differtntial_Transformer import DifferentialTransformer
...
class T_UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        cas = F.sigmoid(out)
        pseudo_label = process_cas(cas)  # 最终 NMS 结果  8 17520 1
        cas, p_class = calculate_res(cas)
        return pseudo_label, p_class, cas
...
class Differrnt_Transformer(torch.nn.Module):
    def __init__(self):
        super(Differrnt_Transformer, self).__init__()

    def forward(self, x, h=8, λ=0.1, λinit = 0.05):
        # Example usage:
        # Example dimensions
        # batch_size = 8
        # seq_len = 17520
        ...
        # λ = 0.1
        # λinit = 0.05
        embedding_dim = 64#x.shape[2]  # x: 8 17520 10
...
class Attention(unittest.TestCase):
    def test_forward_pass(self):
        ...
        model = Differrnt_Transformer().cuda()
        # x = torch.randn(batch_size, length, channels).float().cuda()  # B C L
        x = torch.randint(0, 256, (1, 1024))
